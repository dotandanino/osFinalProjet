...
Graph:: Graph(int numOfVertex){
    ...
    neighborsMatrix.resize(numOfVertex,std::vector<int>(numOfVertex,-1));
}
 ...
 */
Graph::Graph(int numOfVertex ,const std::vector<std::vector<int>> neighborsMatrix){
     if(numOfVertex<=0){
        throw std::runtime_error("you must have positive number of vertex");
    }
    this->numOfVertex=numOfVertex;
...
void Graph::addEdge(int src,int dest,int weight){
    if(src>=numOfVertex || src<0 || dest>=numOfVertex || dest<0 || weight<0){
        throw std::runtime_error("src and dest must be in {0,....,n-1} , weight cant be negative");
    }
    if(src == dest){
        throw std::runtime_error("src and dest cant be the same");
    }
    neighborsMatrix[src][dest]=weight;
    neighborsMatrix[dest][src]=weight;
}

/**
 * @brief Adds an directed edge between two vertices in the graph.
 * @param src The source vertex.
 * @param dest The destination vertex.
 * @param weight the weight of the edge.
 * @throws std::runtime_error if src or dest is out of bounds or if they are the same vertex or they are already connected.
...
bool Graph::isConnected(){
    ...
    while(!bfs.empty()){
        ...
        bfs.pop();
        for(int i=0;i<numOfVertex;i++){
            if(neighborsMatrix[v][i]>=0 && distance[i] == 1){
                distance[i] =0;
                bfs.push(i);
            }
        }
...
vector<int> Graph::findEuler(){
    ...
    vector<vector<int>> copyMat = neighborsMatrix;
    stack<int> s;
    s.push(0);
    while(!s.empty()){
        int v= s.top();
